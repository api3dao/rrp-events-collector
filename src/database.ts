...
export const initDb = async (config: Config, optionalOverrides?: OverrideDatabaseOptions) => {
  ...
  const port = parseInt(optionalOverrides?.port ?? process.env.POSTGRES_POST ?? '65432');
  ...
  const dbConnectResult = await go(operation, {
    ...
    await sendToOpsGenieLowLevel(
      {
        ...
        description: dbConnectResult.error
          ? `Message: ${dbConnectResult.error.message}
      Stack: 
      ${JSON.stringify(dbConnectResult.error.stack, null, 2)}`
